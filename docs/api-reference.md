...
## SpacetimeDB Management

### `GET /api/spacetimedb`
Get database status. Auto-starts if not running.

**Response:** `{ running, pid, port, host, version, database: { exists, tables }, baseUrl }`

### `POST /api/spacetimedb`
Manually start SpacetimeDB.

### `DELETE /api/spacetimedb`
Stop SpacetimeDB.

### `GET /api/spacetimedb/logs`
Get SpacetimeDB process logs.
