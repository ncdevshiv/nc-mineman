...
export function Dashboard() {
	...
	const handleKill = async () => {
		if (!activeServerId) return;
		if (!confirm('Are you sure? This will force-kill the server process without saving. Data may be lost.')) return;
		setLoading(true);
		try {
			const res = await fetch(`/api/servers/${activeServerId}/kill`, { method: 'POST' });
			if (!res.ok) {
				alert('Failed to kill server process');
			} else {
				setStatus('stopped');
				await fetchServers();
			}
